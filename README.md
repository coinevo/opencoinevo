# OpenEvo - a fully open sourced implementation of MyCoinevo backend

In this example [restbed](https://github.com/Corvusoft/restbed/) is used to
demonstrate how to provide Coinevo related JSON REST service. For this purpose,
 a service called Open Coinevo was developed.


OpenCoinevo is an open source implementation of backend of
https://mycoinevo.com/. The frontend, which includes HTML, CSS, JavaScript, was adapted
from (and originally developed by) https://mycoinevo.com/.

Open Coinevo's backend is open sourced, free
to use, host and modify. Additionally, some features were added/changed as compared
to MyCoinevo. They include:

 - google analytics, cloudflare, images and flash were removed.
 - frontend's html and javascript upgrades and cleanups.
 - transaction service fee was set to zero (same as in MyCoinevo).
 - the wallets generated use 25 word mnemonics, fully compatible with official coinevo wallets
(13 word mnemonics generated by MyCoinevo work as usual though).
...
- [http://139.162.60.17:8100](http://139.162.60.17:8100) - MyCoinevo frontend

This is OpenCoinevo running on stagnet network. You can use it to play around with it.
...
[branch](https://github.com/coinevoexamples/opencoinevo/tree/devel).


## Screenshot

![Open Coinevo](https://raw.githubusercontent.com/coinevoexamples/opencoinevo/master/screenshot/screen1.png)


## Host it yourself

The Open Coinevo consists of four components that need to be setup for it to work:
 ...
 - Frontend - it is virtually same as that of MyCoinevo, except before mentioned differences.
  It consists of HTML, CSS, and JavaScript.
 - Coinevo daemon - daemon must be running and fully sync, as this is
 where all transaction data is fetched from and used. Daemon also commits txs
 from the Open Coinevo into the Coinevo network.
 - Backend - fully written in C++. It uses [restbed](https://github.com/Corvusoft/restbed/) to serve JSON REST to the frontend
 and [mysql++](http://www.tangentsoft.net/mysql++/) to interface the database. It also accesses Coinevo blockchain and "talks"
 with Coinevo deamon.
...
Open Coinevo is not as fast as MyCoinevo.
...
Below are example and basic instructions on how to setup up and run Open Coinevo on Ubuntu 16.04.
...
#### Compilation of the OpenCoinevo (don't run it yet)

Once Coinevo was downloaded and compiled, we can download Open Coinevo and compile it.
...
work without database, setup frontend, and synced and running coinevo blockchain.
...
git clone --recursive https://github.com/coinevoexamples/opencoinevo.git
...
# altearnatively can use cmake -DCOINEVO_DIR=/path/to/coinevo_folder ..
...
Create opencoinevo database called `opencoinevo`.

```
cd opencoinevo/sql
docker exec -i ommariadb mysql -uroot -proot < opencoinevo.sql
```

#### PhpMyAdmin (using docker)
A good way to manage/view the opencoinevo database is through the
...
```
docker run --name omhtml -p 80:80 -v /home/mwo/opencoinevo/html:/usr/share/nginx/html:ro -d nginx
```

where `omhtml` is docker container name, `80:80` will expose the frontend
on port 80 of the localhost, and `/home/mwo/opencoinevo/html` is the location on your host computer where the
...
#### mycoinevo-core-js (optional)

OpenCoinevo uses frontend code provided by mycoinevo.com. Among many files
used, the two crtical ones are binary webassamply
[MyCoinevoCoreCpp_WASM.wasm](https://mycoinevo.com/js/lib/mycoinevo_core_js/coinevo_utils/MyCoinevoCoreCpp_WASM.wasm) and
the corresponding JavaScript [mycoinevo-core.js](https://mycoinevo.com/js/lib/mycoinevo-core.js) files.
They are used by [send_coins.js](https://mycoinevo.com/js/controllers/send_coins.js?) for providing
transaction generation functionality.

OpenCoinevo provides these files here: `./html/js/lib`. They were generated using forked  `mycoinevo-core-js` repo:
https://github.com/coinevoexamples/mycoinevo-core-js/tree/opencoinevo

However, you can compile them yourself using the orginal repository located at
https://github.com/mycoinevo/mycoinevo-core-js.

Below are instructions on how it can be done on Arch Linux.

```
git clone https://github.com/mycoinevo/mycoinevo-core-js.git

cd mycoinevo-core-js/
...
# compile mycoinevo-core-js
./bin/build-emcpp.sh

# generate mycoinevo-core.js and MyCoinevoCoreCpp_WASM.wasm 
./bin/package_browser_js

```

The above instructions should produce `mycoinevo-core.js`
and `mycoinevo_core_js/coinevo_utils/MyCoinevoCoreCpp_WASM.wasm`
(both located in `./build` folder), which can
be used in place the files bundled with OpenCoinevo.
...
opencoinevo, Open Coinevo backend service:
  ...
  -c [ --config-file ] arg (=./config/config.json)
                                        Config file path.
  -m [ --coinevo-log-level ] arg (=1)    Coinevo log level 1-4, default is 1.
  -l [ --log-file ] arg (=./opencoinevo.log)
...
Before running `opencoinevo`:
 ...
 - make sure coinevo daemon is running and fully sync. If using testnet or stagenet networks, use coinevo daemon
...
```bash
./opencoinevo -s -c /path/to/config.json
```


## OpenCoinevo JSON REST API

Example JSON REST requests and their responses of [OpenCoinevo](https://github.com/coinevoexamples/opencoinevo) are provided below. The long term goal
...
the OpenCoinevo api does not fully conform to that.

#### get_version

Get version of the OpenCoinevo, its API and coinevo.
...
```json
{
  ...
  "coinevo_version_full": "0.10.3.1-125f823"
}
```
...
Login an existing or a new user into OpenCoinevo.
...
Get random outputs from the coinevo daemon to be used as ring members
...
Submit raw tx in hex generated by the fronted to be relayed to coinevo network.
