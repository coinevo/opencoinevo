...
#define TX_FROM_HEX(hex_string)                                              \
    ...
    ASSERT_TRUE(evoeg::hex_to_tx(hex_string, tx, tx_hash, tx_prefix_hash));  \
    string tx_hash_str = pod_to_hex(tx_hash);                                \
    string tx_prefix_hash_str = pod_to_hex(tx_prefix_hash);                  \
    crypto::public_key tx_pub_key = evoeg::get_tx_pub_key_from_received_outs(tx);    \
...
#define ACC_FROM_HEX(hex_address)                                            \
         ...
         ASSERT_TRUE(this->evo_accounts->select(hex_address, acc));
...
#define ADDR_VIEWKEY_FROM_STRING(address_str, viewkey_str, net_type)         \
    ...
    ASSERT_TRUE(evoeg::addr_and_viewkey_from_string(                         \
                    address_str, viewkey_str,                                \
                    net_type, address, viewkey));

#define SPENDKEY_FROM_STRING(spendkey_str)                                   \
    crypto::secret_key spendkey;                                             \
    ASSERT_TRUE(evoeg::parse_str_secret_key(spendkey_str, spendkey));
...
#define GET_RING_MEMBER_OUTPUTS(ring_member_data_hex)                        \
    ...
    ASSERT_TRUE(evoeg::output_data_from_hex(ring_member_data_hex,            \
...
#define GET_KNOWN_OUTPUTS(known_outputs_csv)                                \
    ...
    ASSERT_TRUE(evoeg::populate_known_outputs_from_csv(                     \
